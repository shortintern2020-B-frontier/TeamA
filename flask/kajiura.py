...
def upload():
    ...
    meal_id,meal_point=[],[]
    print("test")
    for meal in range(len(meal_name)):
    ...
    try:
        ...
        def  judge_budges(meal_id):
            ...
            if meal_count==1:
                change_badges(user_id,1)
                return {'meal_name':meal_name,'badge_level':1}
            # badge_level 2　バッチ5個(仮)
            elif meal_count == 5:
                change_badges(user_id,2)
                return {'meal_name':meal_name,'badge_level':2}
            # badge_level 3　バッチ10個(仮)
            elif meal_count >= 10:
                change_badges(user_id,3)
                return {'meal_name':meal_name,'badge_level':3}
            else:
                badge_level=None
                return {'meal_name':meal_name,'badge_level':badge_level}
        return jsonify(meal_name1=judge_budges(meal_id1),meal_name2=judge_budges(meal_id2),meal_name3=judge_budges(meal_id3),
        meal_name4=judge_budges(meal_id4),meal_name5=judge_budges(meal_id5))
    except Exception as e:
        abort(404, {'code': 'Not found', 'message': str(e)})
